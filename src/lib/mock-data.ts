...
const THANG_KHMAO_DOM_KHLOK_SIZES = [
  "5 x 9",
  "6 x 11",
  "6 x 14",
  "8 x 15",
...
const seedStockIns: AppState["stockIns"] = STOCK_IN_PLAN.map((invoice) => ({
  ...
  total: roundMoney(invoice.items.reduce((sum, item) => sum + item.quantity * item.buyPrice, 0)),
}));
