...
function OrdersPage() {
  ...
  const visibleActiveOrders = orders.filter(
    (order) => order.status !== "completed" && order.status !== "cancelled",
  );
  const activeRouteOrders = useMemo(
    () =>
  ...
  const routeMarketSummaries = useMemo(() => {
    ...
    for (const order of activeRouteOrders) {
  ...
  }, [activeRouteOrders, orderMarkets, state.products]);
  const truckOrders = useMemo(
    () =>
      market === ALL_MARKETS
        ? []
        : activeRouteOrders.filter((order) => orderMarkets.get(order.id) === market),
    [activeRouteOrders, market, orderMarkets],
  );
  ...
  return (
    <div>
      ...
      <PageSection
        ...
        description={
          market === ALL_MARKETS
            ? "Choose one market to prepare one delivery truck route."
        ...
        actions={
          <div className="flex flex-wrap items-center justify-end gap-2 text-sm">
            <span className="rounded-md bg-primary/15 px-3 py-2 font-semibold text-primary">
              {market === ALL_MARKETS ? "Select market" : `${truckOrders.length} active`}
            </span>
      ...
      >
            ...
            {routeMarketSummaries.map((summary) => (
              ...
              >
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  {summary.orderCount} active order(s)
                </p>
              </button>
            ))}
          </div>
        ...
        {market === ALL_MARKETS ? (
          <div className="rounded-md border border-dashed border-border p-6 text-center text-sm text-muted-foreground">
            Select one delivery market above before loading the truck.
          </div>
        ) : loadingSummary.length > 0 ? (
          ...
          <div className="rounded-md border border-dashed border-border p-6 text-center text-sm text-muted-foreground">
            No active products to load for {market}.
          </div>
        )}
      </PageSection>

      <PageSection title="Order Control" description="Filter, confirm, pack, and complete orders.">
